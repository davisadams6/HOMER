//When compiling must do: g++ openloop_tracker.cpp encoder.cpp motor_command.cpp joystick.cc wheel_vel.cpp castor.cpp local2global.cpp parameters.cpp odometry.cpp csv_read_openloop.cpp -o openloop_tracker -Wno-psabi -L/usr/local/lib -lserial
...
int csv_read(std::string &filename, double* traj_time, double* traj_xd, double* traj_yd, double* traj_psid);

int main(int argc, char** argv)
{
	// Read in trajectory
	double traj_time[100000];
        double traj_xd[100000];
        double traj_yd[100000];
        double traj_psid[100000];
        std::string traj = "circle_traj2.csv";
        csv_read(traj,traj_time,traj_xd,traj_yd,traj_psid);
        int i =0;
        while(traj_time[i+1]!=0 && traj_xd[i+1]!=0 && traj_yd[i+1]!=0 && traj_psid[i+1]!=0){
                i++;
        }
	printf("got here\n");
	std::vector<double> t_data(i), xd(i), yd(i), psid(i);
        for(int j=0;j<i;j++){
                t_data[j] = traj_time[j];
                xd[j] = traj_xd[j];
                yd[j] = traj_yd[j];
                psid[j] = traj_psid[j];
                printf("%f      %f      %f      %f\n",t_data[j],xd[j],yd[j],psid[j]);
        }

	//Generate spline
	tk::spline xd_data;
    	xd_data.set_points(t_data,xd);
	tk::spline yd_data;
        yd_data.set_points(t_data,yd);
	tk::spline psid_data;
        psid_data.set_points(t_data,psid);
